...
public class FLACAudioHandler {
	...
	private static final String timeButtonInit = " " + '\u264f' + " " + "00:00"
			+ " " + '\u264f' + " ";
	private static final int[][] FIXED_PREDICTION_COEFFICIENTS = { {}, { 1 },
			{ 2, -1 }, { 3, -3, 1 }, { 4, -6, 4, -1 }, };
	private static final int[] rgbComb = { 0x9933CC, 0x66CC33, 0xFFFF00,
			0x990066, 0x0099FF, 0xFF9900, 0x009933, 0x00CCFF, 0x330099,
			0x333333, 0x33CC33, 0x33CCFF, 0x6600CC, 0x660033, 0x663399,
			0x9900CC, 0xCC3333, 0xFF0099, 0xFF9900 };
	...
	private static class GUIGenerator extends JFrame implements MouseListener,
		...
		private GUIGenerator() {
			...
			timeButton.setText(timeButtonInit);
			timeButton.setPreferredSize(new Dimension(180, 128));
			...
			timeButton.setForeground(new Color(
					rgbComb[(int) (rgbComb.length * Math.random())]));
			...
			timeButton.addKeyListener(this);
			...
			timeButton.enableInputMethods(false);
		...
		private class AudioParser implements Runnable {
			...
			public AudioParser() {
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "Cannot play \"" + songName + "\" on your current OS.\nSample rate: " + handler.getSampleRateInHz() + ", sample depth: " + handler.getBitsPerSample(), "Initialization Failed", JOptionPane.ERROR_MESSAGE);
					filesNotBeenInitialized = true;
